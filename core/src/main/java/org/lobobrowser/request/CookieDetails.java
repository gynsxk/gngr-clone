...
final class CookieDetails {
  ...
  final Optional<Date> getExpiresDate() {
    Optional<Date> expiresDate = Optional.empty();
    if (maxAge != null) {
      if (maxAge <= 0) {
        logger.log(Level.WARNING, "getExpiresDate(): Max-age is negative or zero: " + maxAge + ".");
        expiresDate = Optional.of(new Date(0));
      } else {
        expiresDate = Optional.of(new Date(System.currentTimeMillis() + (maxAge * 1000)));
      }
