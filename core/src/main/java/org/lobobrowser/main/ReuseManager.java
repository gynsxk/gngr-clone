...
public class ReuseManager {
    ...
    public void shutdown() {
        final File appHome = StorageManager.getInstance().getAppHome();
        final File portFile = new File(appHome, PORT_FILE);
        portFile.delete();
    }
    ...
    public void launch(final String[] args, final SSLInfo sslInfo) throws Exception {
        ...
        try {
            ...
            for (int tries = 0; tries < 5; tries++) {
                // Look for running VM
                int port = -1;
                try (final InputStream in = new FileInputStream(portFile);
                ...
                if (port != -1) {
                    try (final Socket s = new Socket(bindHost, port)) {
                ...
                try (final OutputStream out = new FileOutputStream(portFile);
